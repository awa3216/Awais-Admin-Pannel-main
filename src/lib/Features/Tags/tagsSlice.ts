  ...
  async (credentials, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/tags/add-tag",
        credentials,
        {
...
>(
  ...
  async (params, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem("token");
      let queryString = "http://localhost:5000/tags/all-tags";
  ...
  async (
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/tags/tagByID?id=${tagsId}`,
        {
          withCredentials: true,
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async ({ payload, tagsId }: { payload: AddTagsPayload; tagsId: string },
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/tags/edit-tag?id=${tagsId}`,
        payload,
        {
          withCredentials: true,
          headers: {
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5000/tags/delete-tag?id=${id}`,
        {
          withCredentials: true,
          headers: {
            Authorization: `Bearer ${token}`,
